...
def train_lgb_model(df: pd.DataFrame, n_folds: int = 5):
    """LightGBM 학습"""
    print("=" * 60)
    print("🚀 LightGBM 모델 학습 시작 (Phase 3)")
    print("=" * 60)
    ...
    model_path = MODEL_DIR / 'lgb_model_v3.pkl'
    ...
    print("📊 성능 비교")
    print("="*60)
    print(f"Phase 2 (v2): CV 18.81m (참고값)")
    print(f"Phase 3 (v3): CV {np.mean(cv_scores_total):.2f}m")
    
    if np.mean(cv_scores_total) < 18.81:
        improvement = 18.81 - np.mean(cv_scores_total)
        print(f"✅ 개선: -{improvement:.2f}m ({improvement/18.81*100:.1f}%)")
    else:
        print(f"⚠️  악화: +{np.mean(cv_scores_total) - 18.81:.2f}m")
...
if __name__ == '__main__':
    print("🎯 K리그 패스 좌표 예측 - LightGBM v3\n")
