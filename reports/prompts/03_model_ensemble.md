...
## 가중치 최적화 실험 (2024-12-14)

### 실험 설계

**가설:**
1. LightGBM CV 가장 낮음 (18.81m) → 비중 증가?
2. XGBoost CV 가장 높음 (18.88m) → 비중 감소?

**실험 조합:**
- 균등: [0.33, 0.33, 0.33]
- LGB 증가: [0.25, 0.50, 0.25]
- XGB 감소: [0.20, 0.40, 0.40] ← 최적!
- LGB 극대: [0.20, 0.60, 0.20]

### 실험 결과

| 가중치 | LB Score | 개선 |
|--------|----------|------|
| [0.33, 0.33, 0.33] | 17.0293 | baseline |
| [0.25, 0.50, 0.25] | 17.0461 | ❌ +0.017 |
| **[0.20, 0.40, 0.40]** | **17.0111** | ✅ **-0.018** |
| [0.20, 0.60, 0.20] | 17.0594 | ❌ +0.030 |

### 핵심 발견

**1. XGBoost의 역할 재정의**
- CV 성능은 낮지만 다양성 제공
- 완전 배제(0%)보다 소량(20%) 유지가 효과적
- "Zone 관점"이 시계열 모델과 보완적
...
**Bias-Variance-Diversity Tradeoff:**
```
Ensemble Error = Average Error - Diversity

따라서:
- Diversity ↑ → Ensemble Error ↓
- 한 모델 지배 → Diversity ↓ → Error ↑
```

**실제 증명:**
- LGB 40%: Diversity 유지 → 최적
- LGB 60%: Diversity 손실 → 악화
...
1. XGBoost 20%: 다양성 유지
2. LightGBM 40%: 주 예측력
3. CatBoost 40%: 보완적 시계열

**효과:**
- LB 17.029 → 17.011 (-0.018)
- 총 개선: 17.23 → 17.01 (-0.22, 1.3%)
...
### **총 개선 요약**
```
Day 3 AM: 17.23 (XGBoost)
Day 3 PM: 17.13 (XGB+LGB)
Day 4 AM: 17.03 (XGB+LGB+Cat)
Day 4 PM: 17.01 (가중치 최적화)
